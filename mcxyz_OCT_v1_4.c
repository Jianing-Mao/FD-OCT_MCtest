...
double x_d[1000], y_d[1000], z_d[1000];
int k = 0;
int nnnn=0;
...
int main(int argc, const char * argv[]) 
{
    ...
    strcpy(myname, "5ag05Short");      // acquire name from argument of function call by user
    printf("name = %s\n",myname);
	...
	do { 
		...
		if ((i_photon>1000) & (fmod(i_photon, (int)(Nphotons/20))  == 0)) 
		{
    ...
    strcpy(filename,myname);
    strcat(filename,"_x.txt");
    printf("saving %s\n",filename);
    fid = fopen(filename, "w");   /* 3D voxel output */
    for(int i = 0;i<k;i++){
        fprintf(fid, "%0.5f\n",x_d[i]);
    }
